...
void testHotDataAccess() {
    ...
    LRUKcache<int, string> lruk(CAPACITY,CAPACITY,2);
    ...
    // 基类指针指向派生类对象（多态）
    std::array<CachePolicy<int, std::string>*, 2> caches = {&lru, &lruk};
    std::vector<int> hits(2, 0); // 记录三种策略分别缓存命中次数
    std::vector<int> get_operations(2, 0); // 三种策略测试分别get访问缓存总次数
    std::vector<std::string> names = {"LRU", "LRUK"};
